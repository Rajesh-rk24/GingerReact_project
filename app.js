		...
		if(request.body.regOrLogin=="Register") {
			bcrypt.genSalt(10, function(err, salt) {
				bcrypt.hash(request.body.pwd, salt, function(err, hash) {
					...
					var postVars = {username: body.username,email: body.email,phone: body.phone, password: hash, dob: body.dob, reg_date: currentDate};
					// insertion into MySQL
					connection.query("SELECT * FROM signup WHERE username='"+body.username+"'", function(err, res, fields){
						if(err) {
							...
							if(res.length) {
								...
								connection.query("INSERT INTO signup set ?", postVars, function(err, result) {
									if(err) {
